...
<%                                       
	...
	String sql="select * from shop_owners where ownid ='"+id+"'"; //sql语句，查找普通用户商家
	System.out.println(sql);                       //输出到控制台判断数据库语句是否正确
	Statement stmt = con.createStatement();
	ResultSet rs = stmt.executeQuery(sql);               //普通用户：商家结果集
	System.out.println(rs);
	int result=0;
	if(rs.next())                                     //循环查找普通用户：商家结果集
	{
			result=1;                                   //结果为1
	}                                                  
if(result==1)//普通用户：商家表中已存在
{   
	out.println("<script language='javaScript'> alert('该学号已被注册，点击返回！');</script>");
	response.setHeader("Refresh","0;url=own_regist.jsp");
}
if(result==0)
{   
	String sql1="insert into shop_owners(ownid,ownpwd,ownsex,ownname,ownphone,owndormitory,ownnumber,o_profile) values('"+id+"','"+pwd+"','"+sex+"','"+name+"','"+phone+"','"+dormitory+"','"+roomnumber+"','img/regist.jpg')";
	stmt.execute(sql1); //执行sql1语句，插入到owns表
