...
<center>
 ...
 <!--打印结果--> 
  <table class="zong">
    ...
    <th class="text-center" width=80px; scope="col">库存</th>
    <th class="text-center" width=120px; scope="col">商品类别</th>
    <th class="text-center" width=140px; scope="col">操作</th>
  </tr>
 <!--设置显示页码-->
  <%
...
<%
	...
	String sql="select * from goods where ownid='"+str+"'"; //sql语句，查找货物
	...
	if(rs.next()){                                      //按名结果集不为空
		...
		out.print("<img class='picture' src= "+rs.getString("gpicture")+" /></a></td><td>"+rs.getString("gname")+"</td><td>"+"<span class='glyphicon-yen'></span>"
		+rs.getString("gprice")+'元'+"</td><td>"+rs.getString("gsold")+"</td><td>"+rs.getString("gnum")+"</td><td>"+rs.getString("ginfor")+"</td>");
		out.print("<td>&nbsp<a href=update_goods.jsp?gname="+rs.getString("gname")+"&ownid="+rs.getString("ownid")+">更新</a>&nbsp&nbsp<a href=delete_goods.jsp?gname="+rs.getString("gname")+"&ownid="+rs.getString("ownid")+">删除</a>&nbsp</td></tr>");
		i++;
	...
	if(result==0){
		...
		if(dipage==1);//当前是首页
			...
			out.print("<a href=allgoods.jsp?dipage=1>首页</a>,");
			out.print("<a href=allgoods.jsp?dipage="+(dipage-1)+">上一页</a>,");
		}
	    if(dipage==countPage); //当前页是末页
	    else{//当前页不是末页
	    	out.print("<a href=allgoods.jsp?dipage="+(dipage+1)+">下一页</a>,");
	    	out.print("<a href=allgoods.jsp?dipage="+countPage+">末页</a>");
	    }
	    out.print("</td></tr>");
	}
	stmt.close();//关闭数据库
