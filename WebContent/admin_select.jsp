...
<%
	...
	String sql="select * from goods order by ginfor"; //sql语句，查找货物
	...
	if(result==0){
		...
		if(dipage==1);//当前是首页
			...
			out.print("<a href=admin_select.jsp?dipage=1>首页</a>,");
			out.print("<a href=admin_select.jsp?dipage="+(dipage-1)+">上一页</a>,");
		}
	    if(dipage==countPage); //当前页是末页
	    else{//当前页不是末页
	    	out.print("<a href=admin_select.jsp?dipage="+(dipage+1)+">下一页</a>,");
	    	out.print("<a href=admin_select.jsp?dipage="+countPage+">末页</a>");
	    }
	    out.print("</td></tr>");
	}
	stmt.close();//关闭数据库
